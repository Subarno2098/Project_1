...
setwd("C:/Users/49152/Documents/Universitas/Master_EAGLE/MB2/R_scripts/Summer_Weather_output")
...
rasdf <- as.data.frame(rasterHist,xy=T)%>%drop_na()

# get country borders of germany
bnd <- raster::getData("GADM", country='DEU', level=1)
bnd.utm <- spTransform(bnd, CRS(proj4string(rasterHist)))
sger <-  rnaturalearth::ne_countries(country ='germany',returnclass = 'sf') # or in another way
 head(rasdf) 
 plot(sger)
 
 p <- ggplot()+
   geom_raster(aes(x=x,y=y,fill=Year_1998),data=rasdf)+
   transition_states(rasdf$Year_1951)
   
 #--------------------------------------------------------------------------------------------------------------
 library(rasterVis)
 gplot(rasterHist)
 
 gplot(rasterHist) + geom_tile(aes(fill = "Year_1951")) 
   facet_wrap(~ variable) 
